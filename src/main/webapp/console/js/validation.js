...
var regexpShxy = /^[a-zA-Z](?![a-zA-Z]*$)[a-zA-Z0-9\S]{17}$/;
...
//将表单校验方法提取出来，方便直接在jsp中调用
function sumbitForme_click(obj){
    flgs = true;
    var cname = $(obj).attr("fromname");
    ...
    if ($("#idF19").length > 0) {
        if ((obj.value).length == 18) {
            if (!regexpShxy.test($("#idF19").val())) {
                $("#idShxy").html("长度为18位，由字母+数字组").show();
                return false;
            }
        } else {
            $("#idShxy").html("").hide();
...
function checkText($eve) {
    ...
    for (var i = 0; i < msg.length; i++) {
        ...
        if (temp.length > 0) {
            ...
            if(temp.indexOf('not-eq')!=-1){
				tsizearry = temp.split("-");
				if(tsizearry.length>=3){
					tsize = tsizearry[2];
					if(parseInt(value) == parseInt(tsize)){
						$error.addClass("error_tip");
						$error.html("不能为" + tsize+"！");
						flg = false;
						flgs = false;
						$tip.hide();
						$error.show();
            ...
            if (temp.indexOf('max-float') != -1) {
                tsizearry = temp.split("-");
                if (tsizearry.length >= 3) {
                    tsize = tsizearry[2];
                    if (parseFloat(value) > parseFloat(tsize)) {
                        $error.addClass("error_tip");
                        $error.html("大于最大范围值:" + tsize);
                        flg = false;
                        flgs = false;
                        $tip.hide();
                        $error.show();
            ...
            if (temp.indexOf('maxf-size') != -1) {
                ...
                if (tsizearry.length >= 3) {
                    ...
                    if (parseFloat(value) > parseFloat(tsize)) {
                        $error.addClass("error_tip");
                        $error.html("超出最大范围值:" + tsizearry[2]);
                        flg = false;
                        flgs = false;
                        $tip.hide();
                        $error.show();
            ...
            if (temp.indexOf('min-length') != -1) {
                ...
                if (tsizearry.length >= 3) {
                    tsize = tsizearry[2];
                    if (value.length > 0 && value.length < parseInt(tsize)) {
                        $error.addClass("error_tip");
            ...
            if (temp.indexOf('max-precision') != -1) {
                ...
                if (tsizearry.length >= 3) {
                    ...
                    if (_d.indexOf('.') > 0 && precision > tsize) {
                        $error.addClass("error_tip");
            ...
            if (temp == "nameAndAddress") {
                var myreg = /^[a-zA-Z\.0-9\u4e00-\u9fa5]{1,30}$/;
                if (!myreg.test(value)) {
                    $error.addClass("error_tip");
                    $error.html("不能输入非法字符!");
                    flg = false;
...
function isIdCardNot(num, $error) {
    ...
    if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(num))) {
        $error.addClass("error_tip");
        $error.html('输入的身份证号不符合规定！15位号码应全为数字，18位号码末位可以为数字或X。');
        return false;
